...
const IOS_DEEP_LINK_PROMPT_WAIT_MS = 1_000;
...
export async function runScreenshotDemo({
  ...
  try {
    ...
    if (options.proveOpenUrls) {
      ...
      throwIfAborted(signal);
      if (options.settleMs > 0) await sleep(options.settleMs, signal);
      throwIfAborted(signal);
      ...
      if (platform === 'ios') {
        if (typeof client.tap !== 'function') {
          throw new Error('Installed @run-cloud/sdk does not support iOS prompt confirmation.');
        }
        await sleep(Math.max(IOS_DEEP_LINK_PROMPT_WAIT_MS, options.settleMs), signal);
        throwIfAborted(signal);
        log('Confirming the iOS custom-scheme prompt...');
        await client.tap(session.id, IOS_DEEP_LINK_CONFIRMATION.point, {
